...
void Find_Collisions_To_Avoid(void);
...
int Are_Connected_By_A_Joint( OBJECT *d1, OBJECT *d2 ) {

	int onePossibility   = avoidCollisions[ d1->ID * environment->numberOfBodies + d2->ID] == 1;

        int otherPossibility = avoidCollisions[ d2->ID * environment->numberOfBodies + d1->ID] == 1;

	return ( onePossibility || otherPossibility ); 
}
...
{ 
  int i,n;
	...
	// Cancel collisions between capped cylinders.

	if ( 	d1 &&

		d2 &&

		Are_Connected_By_A_Joint(d1,d2) )
 
//		( dGeomGetClass(o1) == dCCylinderClass ) &&

//		( dGeomGetClass(o2) == dCCylinderClass ) )
...
void Read_From_Python(void) {
	...
	Find_Collisions_To_Avoid();
}

void Find_Collisions_To_Avoid(void) {

        avoidCollisions = new int[ environment->numberOfBodies * environment->numberOfBodies ];

	for ( int i = 0; i < environment->numberOfBodies ; i++ )

		for ( int j = 0 ; j < environment->numberOfBodies ; j++ )

			avoidCollisions[ i * environment->numberOfBodies + j ] = 0;

	environment->Find_Collisions_To_Avoid( avoidCollisions );
}
